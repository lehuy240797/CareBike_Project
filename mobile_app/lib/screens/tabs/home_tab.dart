...
const _banners = <_Banner>[
  _Banner('Limited offer', '20% off your first service', 'New riders only · ends Sunday', Icons.redeem),
  _Banner('Free pickup', 'We collect your bike', 'Doorstep pickup within 5 km', Icons.two_wheeler),
  _Banner('Express', 'Oil change in 20 minutes', 'Walk in — no booking needed', Icons.bolt),
];
...
const _quick = <_Quick>[
  ...
  _Quick('Support', Icons.support_agent_rounded, _QuickAction.support), // Help Center chat
];
...
class _HomeTabState extends State<HomeTab> {
          ...
          children: [
            ...
            Row(children: [
              Container(
                ...
                child: Icon(Icons.calendar_today_rounded, size: 17, color: AppColors.primary),
              ),
              const SizedBox(width: 10),
              Text('Choose appointment date', style: AppStyles.section(size: 16)),
            ]),
              ...
              child: CupertinoTheme(
                data: CupertinoThemeData(
                  textTheme: CupertinoTextThemeData(
                    dateTimePickerTextStyle: TextStyle(fontSize: 20, fontWeight: FontWeight.w700, color: AppColors.ink),
                  ),
                ),
            ...
            SizedBox(
              ...
              height: 50,
              child: FilledButton(
                onPressed: () => Navigator.pop(ctx, DateTime(temp.year, temp.month, temp.day)),
          ...
          children: [
            ...
            Row(children: [
              Container(
                padding: const EdgeInsets.all(8),
                decoration: BoxDecoration(color: AppColors.primaryLight, borderRadius: BorderRadius.circular(10)),
                child: Icon(Icons.access_time_rounded, size: 18, color: AppColors.primary),
              ),
              const SizedBox(width: 10),
              Text('Choose appointment time', style: AppStyles.section(size: 16)),
            ]),
            const SizedBox(height: 12),
            SizedBox(
              height: 190,
              child: CupertinoTheme(
                data: CupertinoThemeData(
                  textTheme: CupertinoTextThemeData(
                    dateTimePickerTextStyle: TextStyle(fontSize: 22, fontWeight: FontWeight.w700, color: AppColors.ink),
                  ),
                ),
                child: CupertinoDatePicker(
            ...
            SizedBox(
              ...
              height: 50,
              child: FilledButton(
                onPressed: () => Navigator.pop(ctx, TimeOfDay(hour: temp.hour, minute: temp.minute)),
                child: const Text('Confirm'),
              ),
            ),
          ],
  ...
  Future<void> _submitBooking() async {
    ...
    try {
      ...
      Future.delayed(const Duration(seconds: 3), () {
        if (mounted) { // Check the user is still on this screen
          setState(() {
      ...
      setState(() { _bookingError = 'Something went wrong. Please try again.'; _isBooking = false; });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final name = (auth.mysqlUser?['fullName'] ?? auth.firebaseUser?.email ?? 'Guest').toString();
        ...
        child: Row(
          ...
          children: [
            ...
            Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                Text('CARE', style: GoogleFonts.montserrat(fontSize: 22, fontWeight: FontWeight.w800, fontStyle: FontStyle.italic, color: AppColors.primary, letterSpacing: -0.5)),
                Text('BIKE', style: GoogleFonts.montserrat(fontSize: 22, fontWeight: FontWeight.w800, fontStyle: FontStyle.italic, color: AppColors.ink, letterSpacing: -0.5)),
              ],
            ),
            Stack(
              clipBehavior: Clip.none,
              children: [
                _iconButton(icon: Icons.notifications_rounded, color: AppColors.ink, onTap: () {}),
                Positioned(
                  top: 9, right: 10,
                  child: Container(
                    width: 8, height: 8,
        ...
        children: [
            ...
            child: Column(
              ...
              children: [
                ...
                Text('Hello, $name',
                    ...
                    style: GoogleFonts.poppins(fontSize: 25, fontWeight: FontWeight.w800, letterSpacing: -0.5, color: AppColors.ink)),
              ],
            ),
          ),
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              boxShadow: [BoxShadow(color: AppColors.primaryHover.withValues(alpha: 0.45), blurRadius: 20, offset: const Offset(0, 8))],
            ),
            child: Text(initial, style: GoogleFonts.poppins(color: Colors.white, fontSize: 19, fontWeight: FontWeight.w800)),
          ),
        ],
      ),
    );
    ...
    return Container(
      ...
      child: Stack(
        children: [
          Positioned(
            right: -34, top: -34,
            child: Container(width: 150, height: 150, decoration: BoxDecoration(shape: BoxShape.circle, color: Colors.white.withValues(alpha: 0.13))),
          ),
          Positioned(
            right: 6, bottom: -22,
            child: Icon(b.icon, size: 108, color: Colors.white.withValues(alpha: 0.18)),
          ),
          Padding(
            ...
            child: Column(
              ...
              children: [
                Container(
                  ...
                  child: Text(b.tag.toUpperCase(),
                      style: GoogleFonts.orbitron(fontSize: 9.5, fontWeight: FontWeight.w600, letterSpacing: 1.5, color: Colors.white)),
                ),
                const SizedBox(height: 10),
                ConstrainedBox(
                  constraints: const BoxConstraints(maxWidth: 215),
                  child: Text(b.title, style: GoogleFonts.poppins(color: Colors.white, fontSize: 21, fontWeight: FontWeight.w800, height: 1.12, letterSpacing: -0.5)),
                ),
                const SizedBox(height: 6),
                Text(b.sub, style: const TextStyle(color: Colors.white, fontSize: 12.5, fontWeight: FontWeight.w500)),
              ],
            ),
          ),
        ],
  ...
  void _onQuickTap(_QuickAction action) {
    switch (action) {
      ...
      case _QuickAction.appointments:
              ...
              child: Column(
                children: [
                  Container(
                    ...
                    child: Icon(q.icon, size: 25, color: AppColors.primaryHover),
                  ),
                  const SizedBox(height: 8),
                  Text(q.label, style: TextStyle(fontSize: 10.5, fontWeight: FontWeight.w600, color: AppColors.inkMuted)),
                ],
              ),
            ),
          );
  ...
  Widget _bookingCard() {
    return Container(
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(24),
        border: Border.all(color: AppColors.edge),
        boxShadow: [BoxShadow(color: AppColors.primaryDeep.withValues(alpha: 0.08), blurRadius: 24, offset: const Offset(0, 8))],
      ),
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Container(
                width: 38, height: 38,
                alignment: Alignment.center,
                decoration: BoxDecoration(color: AppColors.primaryMuted, borderRadius: BorderRadius.circular(11)),
                child: Icon(Icons.event_available_rounded, size: 21, color: AppColors.primary),
              ),
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('Book maintenance', style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.w700, color: AppColors.ink, letterSpacing: -0.2)),
                    Text('Reserve a slot in under a minute', style: TextStyle(fontSize: 11.5, color: AppColors.inkMuted, fontWeight: FontWeight.w500)),
                  ],
                ),
              ),
            ],
          ...
          // Branch selector
          if (_branchesLoading)
            _fieldShell(child: Row(children: [
              SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2, color: AppColors.primary)),
              SizedBox(width: 10),
              Text('Loading branches…', style: TextStyle(fontSize: 13, color: AppColors.inkMuted, fontWeight: FontWeight.w500)),
            ]))
          else
            _fieldShell(
              child: Row(
                  ...
                  Expanded(
                    child: DropdownButtonHideUnderline(
                      child: DropdownButton<Branch>(
                        ...
                        isExpanded: true,
                        isDense: true,
                        hint: Text('Select branch', style: TextStyle(fontSize: 13.5, fontWeight: FontWeight.w600, color: AppColors.inkMuted)),
                        icon: Icon(Icons.expand_more_rounded, color: AppColors.inkMuted),
          ...
          Row(
            children: [
              Expanded(child: _pickerTile(Icons.calendar_today_rounded, _dateLabel(), _selectedDate != null, _pickDate)),
              const SizedBox(width: 11),
              Expanded(child: _pickerTile(Icons.schedule_rounded, _timeLabel(), _selectedTime != null, _pickTime)),
            ],
          ),
          const SizedBox(height: 11),
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: [BoxShadow(color: AppColors.primary.withValues(alpha: 0.55), blurRadius: 24, offset: const Offset(0, 12))],
              ),
              child: _isBooking
                  ...
                  : Row(
                      ...
                      children: const [
                        ...
                        SizedBox(width: 8),
                        Text('Book now', style: TextStyle(color: Colors.white, fontSize: 15, fontWeight: FontWeight.w700)),
                      ],
                    ),
            ),
          ),
  ...
  Widget _pickerTile(IconData icon, String label, bool active, VoidCallback onTap) {
    return InkWell(
      onTap: onTap,
      borderRadius: BorderRadius.circular(14),
      child: Container(
        ...
        child: Row(
          children: [
            ...
            Expanded(
              child: Text(label,
                  overflow: TextOverflow.ellipsis,
                  style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: active ? AppColors.ink : AppColors.inkMuted)),
            ),
          ],
        ),
      ),
